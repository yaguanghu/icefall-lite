...
from icefall_lite.graph import ContextGraph, ContextState
from icefall_lite.lm import LmScorer, NgramLm, NgramLmStateCost
from icefall_lite.lm.rnn_lm.model import RnnLmModel
from icefall_lite.lm.transformer_lm.model import TransformerLM
from icefall_lite.utils import DecodingResults, Nbest, add_eos, add_sos, get_texts, get_texts_with_timestamp, one_best_decoding
...
def fast_beam_search_nbest_oracle(
    ...
    tot_scores = levenshtein_alignment.get_tot_scores(use_double_scores=False, log_semiring=False)
    ragged_tot_scores = k2.RaggedTensor(nbest.shape, tot_scores)
...
def fast_beam_search(
    ...
    for t in range(T):
        ...
        if ilme_scale != 0:
            ilme_logits = model.joiner(
                torch.zeros_like(current_encoder_out, device=current_encoder_out.device).unsqueeze(2),
                decoder_out.unsqueeze(1),
                project_input=False,
            )
    ...
    lattice = decoding_streams.format_output(encoder_out_lens.tolist(), allow_partial=allow_partial)
...
def greedy_search(
    ...
    decoder_input = torch.tensor([-1] * (context_size - 1) + [blank_id], device=device,
                                 dtype=torch.int64).reshape(1, context_size)
    ...
    while t < T and sym_per_utt < max_sym_per_utt:
        ...
        # fmt: on
        logits = model.joiner(current_encoder_out, decoder_out.unsqueeze(1), project_input=False)
...
def get_hyps_shape(hyps: List[HypothesisList]) -> k2.RaggedShape:
    ...
    ans = k2.ragged.create_ragged_shape2(row_splits=row_splits, cached_tot_size=row_splits[-1].item())
    return ans
...
def modified_beam_search(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        ys_log_probs = torch.cat([hyp.log_prob.reshape(1, 1) for hyps in A for hyp in hyps])    # (num_hyps, 1)
        ...
        log_probs_shape = k2.ragged.create_ragged_shape2(row_splits=row_splits, cached_tot_size=log_probs.numel())
    ...
    if context_graph is not None:
        ...
        for i, hyps in enumerate(B):
            for hyp in list(hyps):
                context_score, new_context_state = context_graph.finalize(hyp.context_state)
                finalized_B[i].add(
                    Hypothesis(
                        ys=hyp.ys,
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                new_hyp = Hypothesis(ys=new_ys, log_prob=new_log_prob, timestamp=new_timestamp)
                B[i].add(new_hyp)
...
def modified_beam_search_lm_rescore_LODR(
    ...
    LODR_scores = torch.tensor(LODR_scores).to(device) * math.log(10)    # arpa scores are 10-based
    ...
    for lm_scale in lm_scale_list:
        for lodr_scale in LODR_scale_list:
            ...
            tot_scores = (am_scores.values / lm_scale + lm_scores - LODR_scores * lodr_scale)
            ragged_tot_scores = k2.RaggedTensor(shape=am_scores.shape, value=tot_scores)
            max_indexes = ragged_tot_scores.argmax().tolist()
            unsorted_hyps = [candidate_seqs[idx] for idx in max_indexes]
            hyps = []
...
def _deprecated_modified_beam_search(
    ...
    for t in range(T):
        ...
        current_encoder_out = current_encoder_out.expand(decoder_out.size(0), 1, 1, -1)    # (num_hyps, 1, 1, encoder_out_dim)
...
def beam_search(
    ...
    B = HypothesisList()
    B.add(Hypothesis(ys=[-1] * (context_size - 1) + [blank_id], log_prob=0.0, timestamp=[]))
...
def fast_beam_search_with_nbest_rnn_rescoring(
    ...
    for n_scale in ngram_lm_scale_list:
        for rnn_scale in rnn_lm_scale_list:
            ...
            tot_scores = (am_scores.values + n_scale * ngram_lm_scores + rnn_scale * rnn_lm_scores)
            ragged_tot_scores = k2.RaggedTensor(nbest.shape, tot_scores)
            max_indexes = ragged_tot_scores.argmax()
            best_path = k2.index_fsa(nbest.fsa, max_indexes)
...
def modified_beam_search_ngram_rescoring(
    ...
    for batch_size in batch_size_list:
        ...
        ys_log_probs = torch.cat(
            [hyp.log_prob.reshape(1, 1) + hyp.state_cost.lm_score * lm_scale for hyps in A for hyp in hyps])    # (num_hyps, 1)
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                new_hyp = Hypothesis(ys=new_ys, log_prob=new_log_prob, state_cost=state_cost)
                B[i].add(new_hyp)
...
def modified_beam_search_LODR(
    ...
    for batch_size in batch_size_list:
        ...
        ys_log_probs = torch.cat([hyp.log_prob.reshape(1, 1) for hyps in A for hyp in hyps])
        ...
        if len(token_list) != 0:
            ...
            if LM.lm_type == "rnn":
                ...
                tokens_to_score = (torch.nn.utils.rnn.pad_sequence(tokens_list, batch_first=True,
                                                                   padding_value=0.0).to(device).to(torch.int64))
        ...
        for i in range(batch_size):
            ...
            for k in range(len(topk_hyp_indexes)):
                ...
                if new_token not in (blank_id, unk_id):
                    ...
                    hyp_log_prob += (lm_score[new_token] * lm_scale + LODR_lm_scale * current_ngram_score + context_score
                                    )    # add the lm score
...
def modified_beam_search_lm_shallow_fusion(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        lm_scores = torch.cat([hyp.lm_score.reshape(1, -1) for hyps in A for hyp in hyps])

...
from icefall_lite.utils import add_eos, add_sos, get_texts
...
def _intersect_device(
    ...
    if num_fsas <= batch_size:
        return k2.intersect_device(a_fsas, b_fsas, b_to_a_map=b_to_a_map, sorted_match_a=sorted_match_a)
    ...
    for start, end in splits:
        ...
        path_lattice = k2.intersect_device(a_fsas, fsas, b_to_a_map=b_to_a, sorted_match_a=sorted_match_a)
...
class Nbest(object):
    ...
    def __init__(self, fsa: k2.Fsa, shape: k2.RaggedShape) -> None:
        ...
        if fsa.shape[0] != shape.tot_size(1):
            raise ValueError(f"{fsa.shape[0]} vs {shape.tot_size(1)}\n"
    ...
    def from_lattice(
        ...
        path = k2.random_paths(lattice, num_paths=num_paths, use_double_scores=use_double_scores)
        ...
        _, _, new2old = word_seq.unique(need_num_repeats=False, need_new2old_indexes=True)
        ...
        if isinstance(lattice.aux_labels, k2.RaggedTensor):
            ...
            aux_labels, _ = lattice.aux_labels.index(indexes=kept_path.values, axis=0, need_value_indexes=False)
        else:
    ...
    def compute_lm_scores(self) -> k2.RaggedTensor:
        ...
        ragged_lm_scores = k2.RaggedTensor(scores_shape, self.fsa.lm_scores.contiguous())
...
def rescore_with_attention_decoder(
    ...
    if memory_key_padding_mask is not None:
        ...
        # use axis=0 here.
        expanded_memory_key_padding_mask = memory_key_padding_mask.index_select(0, path_to_utt_map)
    else:
    ...
    for n_scale in ngram_lm_scale_list:
        for a_scale in attention_scale_list:
            tot_scores = (am_scores.values + n_scale * ngram_lm_scores.values + a_scale * attention_scores)
            ragged_tot_scores = k2.RaggedTensor(nbest.shape, tot_scores)
            max_indexes = ragged_tot_scores.argmax()
            best_path = k2.index_fsa(nbest.fsa, max_indexes)

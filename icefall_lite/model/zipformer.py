...
from icefall_lite.model import EncoderInterface
from icefall_lite.utils import ScaledLinear  # not as in other dirs.. just scales down initial parameter values.
from icefall_lite.utils import (ActivationBalancer, BasicNorm, DoubleSwish, Identity, MaxEig, ScaledConv1d, Whiten, get_rank,
                                is_jit_tracing, make_pad_mask, penalize_abs_values_gt, random_clamp, softmax)
from icefall_lite.utils.scaling import _diag
...
class Zipformer(EncoderInterface):
    ...
    def __init__(
        ...
        self.encoder_embed = Conv2dSubsampling(num_features,
                                               encoder_dims[0],
                                               dropout=dropout)
        ...
        for i in range(num_encoders):
            ...
            if zipformer_downsampling_factors[i] != 1:
                encoder = DownsampledZipformerEncoder(
                    encoder,
                    input_dim=encoder_dims[i -
                                           1] if i > 0 else encoder_dims[0],
    ...
    def _get_layer_skip_dropout_prob(self):
        ...
        if batch_count > self.warmup_end:
            ...
            return 0.5 - (batch_count / self.warmup_end) * (0.5 -
                                                            min_dropout_prob)
    ...
    def get_feature_masks(self, x: torch.Tensor) -> List[float]:
        ...
        ) or not self.training or torch.jit.is_tracing():
        ...
        frame_mask_max = (torch.rand(num_frames_max,
                                     batch_size,
                                     1,
                                     device=x.device)
        ...
        for i in range(num_encoders):
            ...
            frame_mask = (frame_mask_max.unsqueeze(1).expand(
                num_frames_max, upsample_factor, batch_size,
                1).reshape(num_frames_max * upsample_factor, batch_size, 1))
    ...
    def forward(
        ...
        lengths = (x_lens - 7) >> 1
        assert x.size(0) == lengths.max().item(), (x.shape, lengths,
                                                   lengths.max())
        ...
        for i, (module,
                skip_module) in enumerate(zip(self.encoders,
                                              self.skip_modules)):
            ds = self.zipformer_downsampling_factors[i]
            ...
            if isinstance(k, int):
                ...
                if torch.jit.is_scripting() or torch.jit.is_tracing():
                    x = skip_module(outputs[k], x)
                      ...
                      ) or random.random() > layer_skip_dropout_prob:
                    x = skip_module(outputs[k], x)
            x = module(
                x,
    ...
    def get_bypass_scale(self):
        ...
        if self.batch_count > warmup_period:
            ...
            clamp_min = initial_clamp_min - (
                self.batch_count / warmup_period) * (initial_clamp_min -
                                                     final_clamp_min)
    ...
    def get_dynamic_dropout_rate(self):
        ...
        if self.batch_count > warmup_period:
            ...
            return initial_dropout_rate - (
                ...
                final_dropout_rate) * (self.batch_count / warmup_period)
    ...
    def get_layers_to_drop(self, rnd_seed: int):
        ...
        def get_layerdrop_prob(layer: int) -> float:
            ...
            if batch_count == 0:
                ...
                return initial_layerdrop_prob + t * (final_layerdrop_prob -
                                                     initial_layerdrop_prob)
...
class DownsampledZipformerEncoder(nn.Module):
    ...
    def __init__(self, encoder: nn.Module, input_dim: int, output_dim: int,
                 downsample: int):
...
class AttentionDownsample(torch.nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        src = src.permute(0, 2, 1, 3).reshape(d_seq_len, batch_size,
                                              ds * in_channels)
...
class SimpleCombiner(torch.nn.Module):
    ...
    def __init__(self,
                 dim1: int,
                 dim2: int,
                 min_weight: Tuple[float] = (0.0, 0.0)):
...
class RelPositionMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = ScaledLinear(attention_dim // 2,
                                     embed_dim,
                                     bias=True,
                                     initial_scale=0.05)

        self.in_proj2 = nn.Linear(embed_dim, attention_dim // 2, bias=False)
        self.out_proj2 = ScaledLinear(attention_dim // 2,
                                      embed_dim,
                                      bias=True,
                                      initial_scale=0.05)
    ...
    def multi_head_attention_forward(
        ...
        if torch.jit.is_tracing():
            ...
            pos_weights = pos_weights.reshape(batch_size, num_heads, time1,
                                              seq_len)
        else:
        ...
        assert list(
            attn_output.size()) == [bsz * num_heads, seq_len, head_dim // 2]
    ...
    def forward2(
        ...
        attn_output = (attn_output.transpose(0, 1).contiguous().view(
            seq_len, bsz, self.attention_dim // 2))
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                attn_weights_entropy = (
                    -((attn_weights + 1.0e-20).log() *
                      attn_weights).sum(dim=-1).reshape(
                          bsz, num_heads, seq_len).mean(dim=(0, 2)))
                attn_output = attn_output.reshape(bsz, num_heads, seq_len,
                                                  head_dim)
                ...
                attn_covar = torch.matmul(attn_output.transpose(1, 2),
                                          attn_output) / (bsz * seq_len)
                ...
                in_proj_covar = (self.in_proj2.weight.reshape(
                    num_heads, head_dim, embed_dim)**2).mean(dim=(1, 2))
                out_proj_covar = (self.out_proj2.weight.reshape(
                    embed_dim, num_heads, head_dim)**2).mean(dim=(0, 2))
                logging.info(
...
class PoolingModule(nn.Module):
    ...
    def __init__(self, d_model: int):
        super().__init__()
        self.proj = ScaledLinear(d_model,
                                 d_model,
                                 initial_scale=0.1,
                                 bias=False)
    ...
    def forward(self, x: Tensor, key_padding_mask: Optional[Tensor] = None):
        ...
        if key_padding_mask is not None:
            if torch.jit.is_tracing():
                ...
                pooling_mask = key_padding_mask.logical_not().to(
                    x.dtype)  # (N, T)
            pooling_mask = pooling_mask / pooling_mask.sum(dim=1, keepdim=True)
            pooling_mask = pooling_mask.transpose(0,
                                                  1).contiguous().unsqueeze(-1)
...
class FeedforwardModule(nn.Module):
    ...
    def __init__(self, d_model: int, feedforward_dim: int, dropout: float):
        ...
        self.out_proj = ScaledLinear(feedforward_dim,
                                     d_model,
                                     initial_scale=0.01)
...
class Conv2dSubsampling(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.unsqueeze(
            1)  # (N, T, idim) -> (N, 1, T, idim) i.e., (N, C, H, W)
        x = self.conv(x)
...
class AttentionCombine(nn.Module):
    ...
    def forward(self, inputs: List[Tensor]) -> Tensor:
        ...
        if random.random() < 0.002:
            logging.info(
                f"Average scores are {scores.softmax(dim=1).mean(dim=0)}")
        ...
        if self.training:
            ...
            arange = (torch.arange(num_inputs,
                                   device=scores.device).unsqueeze(0).expand(
            ...
            single_prob_mask = torch.logical_and(apply_single_prob, arange
                                                 < mask_start - 1)
...
def _test_zipformer_main():
    ...
    assert ((seq_len - 7) // 2 + 1) // 2 == f[0].shape[1], (seq_len,
                                                            f.shape[1])
    f[0].sum().backward()
    c.eval()
    f = c(
...
def _test_conv2d_subsampling():
    ...
    encoder_dims = 384
    dropout = 0.1
    encoder_embed = Conv2dSubsampling(num_features,
                                      encoder_dims,
                                      dropout=dropout)

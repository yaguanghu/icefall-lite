...
from icefall_lite.utils import ActivationBalancer
...
class ScaledAdam(BatchedOptimizer):
    ...
    def __init__(
        ...
        assert parameters_names is not None, ("Please prepare parameters_names,"
    ...
    def _get_clipping_scale(self, group: dict, tuples: List[Tuple[Tensor, dict, List[str]]]) -> float:
        ...
        if step % clipping_update_period == 0:
            ...
            quartiles = []
            for n in range(0, 5):
                index = min(clipping_update_period - 1, (clipping_update_period // 4) * n)
            ...
            percent_clipped = (first_state["num_clipped"] * 100.0 /
                               clipping_update_period if "num_clipped" in first_state else 0.0)
    ...
    def _show_gradient_dominating_parameter(self, tuples: List[Tuple[Tensor, dict, List[str]]], tot_sumsq: Tensor):
        ...
        sorted_by_proportion = {k: v for k, v in sorted(all_sumsq_orig.items(), key=lambda item: item[1][0], reverse=True)}
...
class LRScheduler(object):
    ...
    def print_lr(self, is_verbose, group, lr):
        ...
        if is_verbose:
            logging.info(f"Epoch={self.epoch}, batch={self.batch}: adjusting learning rate"
...
class Eden(LRScheduler):
    ...
    def get_lr(self):
        ...
        warmup_factor = (1.0 if self.batch >= self.warmup_batches else 0.5 + 0.5 * (self.batch / self.warmup_batches))
...
def _plot_eden_lr():
    ...
    m = torch.nn.Linear(100, 100)
    parameters_names = []
    parameters_names.append([name_param_pair[0] for name_param_pair in m.named_parameters()])
        ...
        for epoch in range(180):
            ...
            for n, (x, y) in enumerate(train_pairs):
                ...
                if n == 0 and epoch % 5 == 0:
                    ...
                    logging.info(f"Iter {iter}, epoch {epoch}, batch {n}, avg_loss {avg_loss:.4g}, lr={lr:.4e}"

...
from icefall_lite.utils import average_checkpoints
...
def get_texts(best_paths: k2.Fsa, return_ragged: bool = False) -> Union[List[List[int]], k2.RaggedTensor]:
...
def get_texts_with_timestamp(best_paths: k2.Fsa, return_ragged: bool = False) -> DecodingResults:
    ...
    """
    if isinstance(best_paths.aux_labels, k2.RaggedTensor):
        all_aux_shape = (best_paths.arcs.shape().remove_axis(1).compose(best_paths.aux_labels.shape))
        ...
        for cut_id, ref, hyp, time_ref, time_hyp in texts:
            ...
            if len(time_ref) > 0:
                if isinstance(time_ref[0], tuple):
                    ...
                    s = ("[" + ", ".join(["(%0.3f, %.03f)" % (i, j) for (i, j) in time_ref]) + "]")
                else:
            ...
            if len(time_hyp) > 0:
                if isinstance(time_hyp[0], tuple):
                    # each element is <start, end> pair
                    s = ("[" + ", ".join(["(%0.3f, %.03f)" % (i, j) for (i, j) in time_hyp]) + "]")
                else:
                    # each element is a float number
...
def write_error_stats(
    ...
    for cut_id, ref, hyp in results:
        ...
        print(
            f"{cut_id}:\t" + " ".join(
                (ref_word if ref_word == hyp_word else f"({ref_word}->{hyp_word})" for ref_word, hyp_word in ali)),
            file=f,
        )
...
def write_error_stats_with_timestamps(
    ...
    if num_delay > 0:
        if with_end_time:
            ...
            var_delay_start = (sum([(i - mean_delay_start)**2 for i in all_delay_start]) / num_delay)

            all_delay_end = [i[1] for i in all_delay]
            mean_delay_end = sum(all_delay_end) / num_delay
            var_delay_end = (sum([(i - mean_delay_end)**2 for i in all_delay_end]) / num_delay)
    ...
    if enable_log:
        logging.info(f"[{test_set_name}] %WER {tot_errs / ref_len:.2%} "
                     f"[{tot_errs} / {ref_len}, {ins_errs} ins, "
                     f"{del_errs} del, {sub_errs} sub ]")
        logging.info(f"[{test_set_name}] %symbol-delay mean (s): "
...
def get_parameter_groups_with_lrs(model: nn.Module, lr: float, include_names: bool = False) -> List[dict]:
    """
...
def filter_uneven_sized_batch(batch: dict, allowed_max_frames: int):
    ...
    logging.info(f"Filtering uneven-sized batch, original batch size is {N}, "
...
def parse_bpe_timestamps_and_texts(best_paths: k2.Fsa,
                                   sp: spm.SentencePieceProcessor) -> Tuple[List[Tuple[int, int]], List[List[str]]]:
...
def parse_timestamps_and_texts(best_paths: k2.Fsa, word_table: k2.SymbolTable) -> Tuple[List[Tuple[int, int]], List[List[str]]]:
    """Parse timestamps (frame indexes) and texts.
...
def parse_fsa_timestamps_and_texts(
    ...
    if sp is not None:
        ...
        utt_index_pairs, utt_words = parse_bpe_timestamps_and_texts(best_paths=best_paths, sp=sp)
    elif word_table is not None:
        assert sp is None, "sp is not needed if word_table is provided."
        utt_index_pairs, utt_words = parse_timestamps_and_texts(best_paths=best_paths, word_table=word_table)
    else:
...
def is_cjk(character):
    ...
    """
    return any([
        start <= ord(character) <= end for start, end in [
            (4352, 4607),
            (11904, 42191),
            (43072, 43135),
            (44032, 55215),
...
def num_tokens(token_table: k2.SymbolTable, disambig_pattern: str = re.compile(r"^#\d+$")) -> int:

...
from icefall_lite.model.zipformer import PoolingModule
from icefall_lite.utils.scaling import ActivationBalancer, BasicNorm, Whiten
...
class NonScaledNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scales = (torch.mean(x * x, dim=self.channel_dim, keepdim=True) + self.eps_exp).pow(-0.5)
        return x * scales

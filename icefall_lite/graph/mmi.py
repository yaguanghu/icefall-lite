...
from icefall_lite.graph.mmi_graph_compiler import MmiTrainingGraphCompiler
...
def _compute_mmi_loss_exact_optimized(
    ...
    num_den_graphs_indexes = (torch.stack([num_graphs_indexes, den_graphs_indexes]).t().reshape(-1).to(device))
...
def _compute_mmi_loss_exact_non_optimized(
    ...
    num_lats = k2.intersect_dense(num_graphs, dense_fsa_vec, output_beam=beam_size, max_arcs=2147483600)
    den_lats = k2.intersect_dense(den_graphs, dense_fsa_vec, output_beam=beam_size, max_arcs=2147483600)

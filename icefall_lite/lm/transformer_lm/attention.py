...
from icefall_lite.utils import (ActivationBalancer, BasicNorm, DoubleSwish, ScaledConv1d, ScaledConv2d, ScaledLinear,
                                is_jit_tracing)
...
class RelPositionMultiheadAttention(nn.Module):
    ...
    def rel_shift(self, x: Tensor, left_context: int = 0) -> Tensor:
        ...
        if not is_jit_tracing():
            assert (n == left_context + 2 * time1 - 1), f"{n} == {left_context} + 2 * {time1} - 1"
    ...
    def multi_head_attention_forward(
        ...
        if attn_mask is not None:
            assert (attn_mask.dtype == torch.float32 or attn_mask.dtype == torch.float64 or attn_mask.dtype == torch.float16
                    or attn_mask.dtype == torch.uint8 or attn_mask.dtype

...
from icefall_lite.utils import AttributeDict, average_checkpoints, load_checkpoint, str2bool
...
class LmScorer(torch.nn.Module):
    ...
    def add_arguments(cls, parser):
        ...
        # Now transformers
        parser.add_argument("--transformer-lm-exp-dir", type=str, help="Directory of transformer LM exp")
    ...
    def get_lm(self, lm_type: str, device, params: AttributeDict) -> torch.nn.Module:
        ...
        if lm_type == "rnn":
            ...
            if params.lm_avg == 1:
                load_checkpoint(f"{params.lm_exp_dir}/epoch-{params.lm_epoch}.pt", model)
                model.to(device)
            else:
...
if __name__ == "__main__":
    ...
    x = (torch.tensor([[1, 4, 19, 256, 77], [1, 4, 19, 256, 77]]).to(device).to(torch.int64))

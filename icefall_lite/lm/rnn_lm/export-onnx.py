...
from icefall_lite.utils.utils import AttributeDict, str2bool
...
class RnnLmModelWrapper(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_lens: torch.Tensor) -> torch.Tensor:
        ...
        sos_tensor = torch.full((1, ), fill_value=self.sos_id,
                                dtype=x.dtype).expand(N, 1)
        ...
        return (self.model(x=sos_x, y=x_eos,
                           lengths=x_lens + 1).to(torch.float32).sum(dim=1))
...
def export_without_state(
    ...
    N = 1
    L = 20
    x = torch.randint(low=1,
                      high=params.vocab_size,
                      size=(N, L),
                      dtype=torch.int64)
...
def main():
    ...
    if params.iter > 0:
        filenames = find_checkpoints(params.exp_dir,
                                     iteration=-params.iter)[:params.avg]
        if len(filenames) == 0:
            raise ValueError(

...
from icefall_lite.utils import add_eos, add_sos, make_pad_mask
...
class RnnLmModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.output_linear = torch.nn.Linear(in_features=hidden_dim, out_features=vocab_size)
    ...
    def streaming_forward(
        ...
        nll_loss = F.cross_entropy(logits.reshape(-1, self.vocab_size), y.reshape(-1), reduction="none")
    ...
    def score_token(self, x: torch.Tensor, x_lens: torch.Tensor, state=None):
        ...
        if state:
            ...
            h = torch.zeros(self.rnn.num_layers, batch_size, self.rnn.hidden_size).to(device)
            c = torch.zeros(self.rnn.num_layers, batch_size, self.rnn.hidden_size).to(device)

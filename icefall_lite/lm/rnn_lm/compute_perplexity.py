...
from icefall_lite.utils.checkpoint import average_checkpoints, find_checkpoints, load_checkpoint
from icefall_lite.utils.utils import AttributeDict, setup_logger, str2bool
...
def main():
    ...
    if params.iter > 0:
        setup_logger(f"{params.exp_dir}/log-ppl/log-ppl-iter-{params.iter}-avg-{params.avg}")
    else:
    ...
    if params.iter > 0:
        ...
        if len(filenames) == 0:
            ...
            raise ValueError(f"Not enough checkpoints ({len(filenames)}) found for"
    ...
    num_param_requires_grad = sum([p.numel() for p in model.parameters() if p.requires_grad])

    logging.info(f"Number of model parameters: {num_param}")
    logging.info(f"Number of model parameters (requires_grad): "
    ...
    logging.info(f"total nll: {tot_loss}, num tokens: {num_tokens}, "

...
from icefall_lite.model.decoder import Decoder
from icefall_lite.model.encoder_interface import EncoderInterface
from icefall_lite.model.model import Transducer
from icefall_lite.model.zipformer import Zipformer
from icefall_lite.utils import (AttributeDict, Eden, Joiner, LRScheduler, MetricsTracker, ScaledAdam, byte_encode, cleanup_dist,
                                diagnostics, filter_uneven_sized_batch, get_env_info, load_checkpoint, register_inf_check_hooks,
                                remove_checkpoints)
from icefall_lite.utils import save_checkpoint as save_checkpoint_impl
from icefall_lite.utils import (save_checkpoint_with_global_batch_idx, setup_dist, setup_logger, str2bool, tokenize_by_CJK_char,
                                update_averaged_model)
from icefall_lite.utils.asr_datamodule import AishellAsrDataModule
...
def set_batch_count(model: Union[nn.Module, DDP], batch_count: float) -> None:
    ...
    for module in model.modules():
        if hasattr(module, "batch_count"):
            module.batch_count = batch_count    # type: ignore
...
def get_encoder_model(params: AttributeDict) -> EncoderInterface:
    ...
    encoder = Zipformer(
        ...
        zipformer_downsampling_factors=to_int_tuple(params.zipformer_downsampling_factors),    # type: ignore
        encoder_dims=to_int_tuple(params.encoder_dims),    # type: ignore
        attention_dim=to_int_tuple(params.attention_dims),    # type: ignore
        encoder_unmasked_dims=to_int_tuple(params.encoder_unmasked_dims),    # type: ignore
        nhead=to_int_tuple(params.nhead),    # type: ignore
        feedforward_dim=to_int_tuple(params.feedforward_dims),    # type: ignore
        cnn_module_kernels=to_int_tuple(params.cnn_module_kernels),    # type: ignore
        num_encoder_layers=to_int_tuple(params.num_encoder_layers),    # type: ignore
    )
    return encoder
...
def compute_loss(
    ...
    y = sp.encode(texts, out_type=int)    # type: ignore
...
def compute_validation_loss(
    ...
    model: Union[nn.Module, DDP],
    sp: spm.SentencePieceProcessor,
    valid_dl: torch.utils.data.DataLoader,    # type: ignore
    world_size: int = 1,
) -> MetricsTracker:
...
def train_one_epoch(
    ...
    sp: spm.SentencePieceProcessor,
    train_dl: torch.utils.data.DataLoader,    # type: ignore
    valid_dl: torch.utils.data.DataLoader,    # type: ignore
    scaler: GradScaler,
    model_avg: Optional[nn.Module] = None,
...
def run(rank, world_size, args):
    ...
    train_dl = aishell.train_dataloaders(train_cuts, sampler_state_dict=sampler_state_dict)

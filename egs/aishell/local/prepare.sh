...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  # to $dl_dir/aishell
  if [ ! -f ${manifests_dir}/.aishell_manifests.done ]; then
    [ ! -d ${manifests_dir} ] && mkdir -p ${manifests_dir}
    lhotse prepare aishell $dl_dir/aishell "${manifests_dir}"
    touch ${manifests_dir}/.aishell_manifests.done
  fi
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  # to data/musan
  if [ ! -f ${manifests_dir}/.musan_manifests.done ]; then
    log "It may take 6 minutes"
    [ ! -d ${manifests_dir} ] && mkdir -p ${manifests_dir}
    lhotse prepare musan $dl_dir/musan ${manifests_dir}
    touch ${manifests_dir}/.musan_manifests.done
  fi
fi

if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  log "Stage 3: Compute fbank for aishell"
  if [ ! -f ${feats_dir}/.aishell.done ]; then
    [ ! -d ${feats_dir} ] && mkdir -p ${feats_dir}
    python -m icefall_lite.bin.compute_fbank_feats --srcdir "${manifests_dir}" --outdir "${feats_dir}" \
      --prefix "aishell" --suffix "jsonl.gz" --dataset_parts train dev test \
      --perturb-speed true || exit 1;
    touch ${feats_dir}/.aishell.done
  fi
fi

if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  log "Stage 4: Compute fbank for musan"
  if [ ! -f ${feats_dir}/.musan.done ]; then
    [ ! -d ${feats_dir} ] && mkdir -p ${feats_dir}
    python -m icefall_lite.bin.compute_fbank_feats --srcdir "${manifests_dir}" --outdir "${feats_dir}" \
      --prefix "musan" --suffix "jsonl.gz" --dataset_parts music speech noise \
      --perturb-speed false || exit 1;
    touch ${feats_dir}/.musan.done
  fi
fi
...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  ...
  [ ! -d ${lang_phone_dir} ] && mkdir -p $lang_phone_dir

  (echo '!SIL SIL'; echo '<SPOKEN_NOISE> SPN'; echo '<UNK> SPN'; ) |
    cat - $dl_dir/aishell/resource_aishell/lexicon.txt |
    sort | uniq > $lang_phone_dir/lexicon.txt

  python -m icefall_lite.lexicon.generate_unique_lexicon --lang-dir $lang_phone_dir || exit 1;

  if [ ! -f $lang_phone_dir/L_disambig.pt ]; then
    python -m icefall_lite.lexicon.prepare_lang --lang-dir $lang_phone_dir || exit 1;
  fi
  ...
  if [ ! -f $lang_phone_dir/transcript_tokens.txt ]; then
    python -m icefall_lite.bin.convert_transcript_words_to_tokens \
      --lexicon $lang_phone_dir/uniq_lexicon.txt \
      --transcript $lang_phone_dir/transcript_words.txt \
      --oov "<UNK>" \
      > $lang_phone_dir/transcript_tokens.txt || exit 1;
  fi

  if [ ! -f $lang_phone_dir/P.arpa ]; then
    python -m icefall_lite.lm.make_kn_lm \
      -ngram-order 2 \
      -text $lang_phone_dir/transcript_tokens.txt \
      -lm $lang_phone_dir/P.arpa || exit 1;
  fi
  ...
  if [ ! -f $lang_phone_dir/P.fst.txt ]; then
    python -m kaldilm \
      ...
      --disambig-symbol='#0' \
      --max-order=2 \
      $lang_phone_dir/P.arpa > $lang_phone_dir/P.fst.txt || exit 1;
  fi
fi
...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  ...
  [ ! -d ${lang_char_dir} ] mkdir -p $lang_char_dir
  ...
  if [ ! -f $lang_char_dir/L_disambig.pt ]; then
    python -m icefall_lite.lexicon.prepare_char --lang-dir $lang_char_dir || exit 1;
  fi
fi
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
  ...
  for vocab_size in ${vocab_sizes[@]}; do
    ...
    if [ ! -f $lang_dir/bbpe.model ]; then
      python -m icefall_lite.lexicon.train_bbpe_model \
        --lang-dir $lang_dir \
        --vocab-size $vocab_size \
        --transcript $lang_dir/text || exit 1;
    fi

    if [ ! -f $lang_dir/L_disambig.pt ]; then
      python -m icefall_lite.lexicon.prepare_lang_bbpe --lang-dir $lang_dir || exit 1;
    fi
  done
fi
...
if [ $stage -le 8 ] && [ $stop_stage -ge 8 ]; then
  ...
  lm_dir=data/lm
  [ ! -d ${lm_dir} ] && mkdir -p ${lm_dir}

  # Train LM on transcripts
  if [ ! -f ${lm_dir}/3-gram.unpruned.arpa ]; then
    python -m icefall_lite.lm.make_kn_lm \
      -ngram-order 3 \
      -text $lang_char_dir/transcript_words.txt \
      -lm ${lm_dir}/3-gram.unpruned.arpa || exit 1;
  fi

  # We assume you have install kaldilm, if not, please install
  # it using: pip install kaldilm
  if [ ! -f ${lm_dir}/G_3_gram_char.fst.txt ]; then
    # It is used in building HLG
    python -m kaldilm \
      --read-symbol-table="$lang_phone_dir/words.txt" \
      --disambig-symbol='#0' \
      --max-order=3 \
      ${lm_dir}/3-gram.unpruned.arpa > ${lm_dir}/G_3_gram_phone.fst.txt || exit 1;

    python -m kaldilm \
      --read-symbol-table="$lang_char_dir/words.txt" \
      --disambig-symbol='#0' \
      --max-order=3 \
      ${lm_dir}/3-gram.unpruned.arpa > ${lm_dir}/G_3_gram_char.fst.txt || exit 1;
  fi
fi

if [ $stage -le 9 ] && [ $stop_stage -ge 9 ]; then
  log "Stage 9: Compile LG & HLG"
  python -m icefall_lite.graph.compile_hlg --lang-dir $lang_phone_dir --lm G_3_gram_phone || exit 1;
  python -m icefall_lite.graph.compile_hlg --lang-dir $lang_char_dir --lm G_3_gram_char || exit 1;
  for vocab_size in ${vocab_sizes[@]}; do
    lang_dir=data/lang_bbpe_${vocab_size}
    python -m icefall_lite.graph.compile_hlg --lang-dir $lang_dir --lm G_3_gram_char || exit 1;
  done

  python -m icefall_lite.graph.compile_lg --lang-dir $lang_phone_dir --lm G_3_gram_phone || exit 1;
  python -m icefall_lite.graph.compile_lg --lang-dir $lang_char_dir --lm G_3_gram_char || exit 1;
  for vocab_size in ${vocab_sizes[@]}; do
    lang_dir=data/lang_bbpe_${vocab_size}
    python -m icefall_lite.graph.compile_lg --lang-dir $lang_dir --lm G_3_gram_char || exit 1;
  done
fi
...
if [ $stage -le 10 ] && [ $stop_stage -ge 10 ]; then
  ...
  # training words
  python -m icefall_lite.lm.prepare_char_lm_training_data \
    --lang-char data/lang_char \
    --lm-data $dl_dir/lm/aishell-train-word.txt \
    --lm-archive $out_dir/lm_data.pt || exit 1;
  ...
  python -m icefall_lite.lm.prepare_char_lm_training_data \
    ...
    --lm-archive $out_dir/lm_data_valid.pt || exit 1;
  ...
  python -m icefall_lite.lm.prepare_char_lm_training_data \
    ...
    --lm-archive $out_dir/lm_data_test.pt || exit 1;
fi
...
if [ $stage -le 11 ] && [ $stop_stage -ge 11 ]; then
  ...
  python -m icefall_lite.lm.sort_lm_training_data \
    --in-lm-data $out_dir/lm_data.pt \
    --out-lm-data $out_dir/sorted_lm_data.pt \
    --out-statistics $out_dir/statistics.txt || exit 1;

  python -m icefall_lite.lm.sort_lm_training_data\
    --in-lm-data $out_dir/lm_data_valid.pt \
    --out-lm-data $out_dir/sorted_lm_data-valid.pt \
    --out-statistics $out_dir/statistics-valid.txt || exit 1;

  python -m icefall_lite.lm.sort_lm_training_data \
    --in-lm-data $out_dir/lm_data_test.pt \
    --out-lm-data $out_dir/sorted_lm_data-test.pt \
    --out-statistics $out_dir/statistics-test.txt || exit 1;
fi

if [ $stage -le 12 ] && [ $stop_stage -ge 12 ]; then
  log "Stage 11: Train RNN LM model"
  python -m icefall_lite.lm.rnnlm.train \
    --start-epoch 0 \
    --world-size 1 \
    --num-epochs 20 \
    --use-fp16 0 \
